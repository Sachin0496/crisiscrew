...
import { classify, extractEntities, type PrototypeVectors } from "./enrich";
import { evaluateGates } from "./gates";
import { DEFAULT_PROTOTYPES, type Prototypes } from "./prototypes";

type Stored = { ticket: Ticket; vector: Vector; signal: SignalView };
type OpenIncident = { id: string; members: Set<string>; centroid: Vector; lastActivity: number };
...
/** The Pattern Agent's detection logic (design section 5). Pure: no I/O besides the embedder. */
export class PatternEngine {
  ...
  async init(): Promise<void> {
    ...
    const surfaceEntries = Object.entries(p.surfaces) as [Exclude<Surface, "other">, string[]][];
  ...
  async ingest(ticket: Ticket): Promise<PatternResult> {
    ...
    const [vector] = await this.embedder.embed([ticketText(ticket)]);
    if (!vector) throw new Error("embedder returned no vector");

    const classification = classify(vector, this.prototypeVectors, this.cfg.surfaceMin);
    const signal: SignalView = {
      ticketId: ticket.id,
      ...classification,
      entities: extractEntities(ticketText(ticket)),
    };
    this.stored.set(ticket.id, { ticket, vector, signal });
    ...
    const active = this.order.filter((id) => {
      ...
      return s.ticket.receivedAt >= windowStart && s.ticket.receivedAt <= now;
    });

    const nearest = active
      .filter((id) => id !== ticket.id)
      .map((id) => ({ ticketId: id, similarity: cosine(vector, this.stored.get(id)!.vector) }))
      .sort((a, b) => b.similarity - a.similarity)
      .slice(0, 3);

    const joinIncidentId = signal.isFailure ? this.matchIncident(vector, now) : undefined;
    if (joinIncidentId) {
      const incident = this.incidents.get(joinIncidentId)!;
      incident.members.add(ticket.id);
      incident.centroid = meanVector([...incident.members].map((id) => this.stored.get(id)!.vector));
      incident.lastActivity = now;
      const candidate = this.describe([...incident.members]);
      return { signal, nearest, candidate, fires: false, joinIncidentId };
    }
  ...
  attachIncident(incidentId: string, memberIds: readonly string[]): void {
    const vectors = memberIds.map((id) => this.stored.get(id)?.vector).filter((v): v is Vector => Boolean(v));
    if (vectors.length === 0) return;
    const lastActivity = Math.max(...memberIds.map((id) => this.stored.get(id)?.ticket.receivedAt ?? 0));
    this.incidents.set(incidentId, { id: incidentId, members: new Set(memberIds), centroid: meanVector(vectors), lastActivity });
  }

  private matchIncident(vector: Vector, now: number): string | undefined {
    let best: { id: string; similarity: number } | undefined;
    for (const incident of this.incidents.values()) {
      if (now - incident.lastActivity > INCIDENT_JOIN_WINDOW) continue;
      const similarity = cosine(vector, incident.centroid);
  ...
  private describe(memberIds: string[]): ClusterView {
    ...
    const cohesion = members.length > 1 ? meanPairwiseSimilarity(ids, (a, b) => this.similarity(a, b)) : 1;
