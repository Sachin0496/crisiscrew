...
| Technical execution | Typed contracts shared by server and UI, an event-sourced UI, an idempotent recovery pass orchestrated as LangGraph workflows with span-level traces (local and LangSmith), guardrails with an adversarial safety eval, 310 tests, CI, a hash-chained audit log, evaluations on held-out splits, Laya run against a real server, and Freshworks adapters tested against fakes of their APIs |
