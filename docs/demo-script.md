...
3. Start the production build (not the dev server):
   ...
   pnpm start
   ```
4. Open http://localhost:8787 in the browser, full screen at 110–125% zoom. The wiring badge in the top bar should read **1 live · 6 sandbox · 3 off**: the embedding model is live, and every other port is sandbox or off.
...
Choose **Checkout release v4.21.7 breaks payments**, set **2× speed**, and click **▶ Run replay**.

Point at the screen as it happens:
- **Incoming tickets:** each ticket is tagged with its product area and marked as a failure report or a question. The first three are ordinary questions and stay out.
- **Pattern Agent · correlation:** the similarity bar and the four gates. "My checkout keeps loading forever", "UPI isn't working", "Payment failed but bank shows debit": no shared keywords.
...
- **Five agents:** the Investigator and the Recovery Agent start in parallel.
- **Investigator · root cause:** the ranking.
...
- **Recovery Agent · customers:** 8 linked tickets and 23 affected customers, 15 of whom never wrote in. Each customer is updated through a channel they agreed to. The two voice updates show as *prepared*, because voice isn't wired.

### 3. The human decision (2:15–2:45)

The **Handoff Agent · human decision** card shows ₹11,500 (23 × ₹500) against a ₹5,000 authority limit.
...
Type `5000` and click **Modify**. The Handoff Agent issues exactly ₹5,000, and the gate would refuse the original ₹11,500.

### 4. Restraint (2:45–3:30)

Choose **Look-alike burst: checkout questions**, set **10× speed**, and click **▶ Run replay**.
...
The correlation card refuses, in plain words: **4 of 5 are questions, not failures**. (If asked why 5 and not 6: the coupon question is tagged refunds and billing, so it falls outside the group.)
...
Scroll to **Governance**. The audit chain shows as verified. Open the **Who may do what** tab for the permission matrix.
...
```
The reply is **Refused by the CrisisCrew policy gate: Pattern Agent is not allowed to call link_ticket_to_incident**, and the refusal appears at the top of the audit log in red.
...
> "Stage 1 was a scripted prototype. This is the real system. The Freshdesk, GitHub, Razorpay and ElevenLabs integrations are designed and listed. The badge shows what's live today: the engine, the agents, the gate and the model. The world they act on is a sandbox."
...
Click **Live mode**, then type these four complaints into the box under **Incoming tickets** within about five minutes. They were rehearsed and open an incident on the fourth, naming checkout-service v4.21.7 at 97%:
...
- **"Is this live or scripted?"** The engine, agents, gate and model are live. The world they read (releases, gateway status, error rates, orders) is a sandbox scenario, and the wiring badge says so. Every number is computed; `pnpm eval` reproduces them.
