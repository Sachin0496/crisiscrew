...
Generated by `pnpm eval` on 2026-09-24, with `Xenova/all-MiniLM-L6-v2` and the thresholds in `config/policy.json`. The runs are seeded, so the same command gives the same numbers. No model is loaded: every text is read from the committed embedding cache.
